...
//  test save file method\n
  ...
  ok = loader.saveFile("TESTFILE1", codeList);  
  assert (ok == true);
  ...
  ok = loader.saveFile("TESTFILE2", codeList);  
  assert (ok == true);
  assert(strcmp(loader.msg, "") == 0);

//  test bad file name  ok = loader.saveFile("TESTFILE2", codeList);  

printf("test write bad file\n");
ok = loader.saveFile("TEST//FILE.", codeList);  
  assert (ok == false);
  ...
  codeList.clear();
  ok = loader.loadFile("TESTFILE1", codeList);
  assert (ok == true);
  assert (codeList.count == 3);
...
assert(strcmp(codeList.getText(2), "PRINT A + B")==  0);
  ...
  //  test read d missing file
  printf("testing read missing file\n");
  ok = loader.loadFile("XYZZY", codeList);
  assert (ok == false);
